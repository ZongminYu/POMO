...
class TSPTrainer_ppo:
    ...
    def _train_one_batch(self, batch_size):
        ...
        # Loss
        ###############################################
        advantage = reward - reward.float().max(dim=1, keepdims=True)[0] #公式中的序号2，当前轨迹的奖励和平均奖励之差
        # shape: (batch, pomo)
        max_index = reward.argmax(dim=1, keepdims=True)

        # self.logger.info("max Index: {}, reward: {}, advantage: {}".format(max_index, reward, advantage))

        prob = prob_list.sum(dim=2) #公式中的序号3.表示每个样本的每个模式(pomo)下所有时间步(t = problem_size)的对数概率之和。
        # size = (batch, pomo)

        log_prob = prob.log()  # log(π(a|s))
        ...
        # self.logger.info("prob_list: {}, prob: {}, log_prob: {}, ratio: {}".format(prob_list, prob, log_prob, ratio))

        loss = -advantage * ratio  # Minus Sign: To Increase REWARD
        # shape: (batch, pomo)
