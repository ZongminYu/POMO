...
class TSPModel_ppo_limit(nn.Module):
    ...
    def forward(self, state):
        ...
        if state.current_node is None:
            ...
            if self.training or self.model_params['eval_type'] == 'softmax':
                while True:
                    #使用多项式分布（multinomial distribution）从 probs 中采样一个动作
                    ...
                    if (prob != 0).all():  #确保采样的动作是有效的（即对应的概率不为零）
                        break
...
#从编码后的节点张量 encoded_nodes 中提取特定节点的编码
